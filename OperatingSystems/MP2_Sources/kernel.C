...
void test_kernel_reserves_info_frames(ContFramePool* kernel_pool);
void test_allocate_and_free_contiguous(ContFramePool* pool);
void test_big_alloc_and_exhaustion(ContFramePool* pool, unsigned long pool_size);
...
/*  // In later machine problems, we will be using two pools. You may want to comment this out and test 
    ...
    // test_kernel_reserves_info_frames(&kernel_mem_pool);
    // test_allocate_and_free_contiguous(&kernel_mem_pool);
    // test_big_alloc_and_exhaustion(&kernel_mem_pool, KERNEL_POOL_SIZE);
...
void test_kernel_reserves_info_frames(ContFramePool* kernel_pool){
    Console::puts("Testing internal metadata reservation\n");
    unsigned long r = ContFramePool::needed_info_frames(KERNEL_POOL_SIZE);
    unsigned long f = kernel_pool->get_frames(1);
    assert(f != 0);
    Console::puts("first kernel alloc -> "); Console::puti((int)f); Console::puts("\n");
    assert(f >= KERNEL_POOL_START_FRAME + (int)r);
    ContFramePool::release_frames(f);
    Console::puts("released -> "); Console::puti((int)f); Console::puts("\n");
    Console::puts("Testing internal metadata reservation done!\n\n");
}

void test_allocate_and_free_contiguous(ContFramePool* pool){
    Console::puts("Testing contiguous allocation and release\n");
    unsigned long n = 6;
    unsigned long s = pool->get_frames((unsigned)n);
    assert(s != 0);
    Console::puts("alloc "); Console::puti((int)n); Console::puts(" -> "); Console::puti((int)s); Console::puts("\n");
    int* p = (int*)(s * (4 KB));
    for (unsigned long i = 0; i < n * (1 KB); i++) p[i] = 0x3344;
    for (unsigned long i = 0; i < n * (1 KB); i++) assert(p[i] == 0x3344);
    ContFramePool::release_frames(s);
    Console::puts("released -> "); Console::puti((int)s); Console::puts("\n");
    Console::puts("Testing contiguous allocation and release done!\n\n");
}

void test_big_alloc_and_exhaustion(ContFramePool* pool, unsigned long pool_size){
    Console::puts("Testing exact fit and exhaustion\n");
    unsigned long want = pool_size, big = 0;
    while (want > 0){ big = pool->get_frames((unsigned)want); if (big) break; want--; }
    assert(big != 0);
    Console::puts("alloc big ("); Console::puti((int)want); Console::puts(") -> "); Console::puti((int)big); Console::puts("\n");
    unsigned long fail = pool->get_frames(1);
    Console::puts("alloc 1 after big -> "); Console::puti((int)fail); Console::puts("\n");
    assert(fail == 0);
    ContFramePool::release_frames(big);
    Console::puts("released big -> "); Console::puti((int)big); Console::puts("\n");
    Console::puts("Testing exact fit and exhaustionn done\n\n");
}
